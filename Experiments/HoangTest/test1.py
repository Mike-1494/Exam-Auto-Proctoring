...
while True:
    ...
    for idx, kpt in enumerate(results1[0].keypoints[0]):
        print(f"Keypoint {idx}: ({int(kpt.xy[0][0])}, {int(kpt.xy[0][1])})")
        #annotated_frame = cv2.putText(annotated_frame, f"{idx}:({int(kpt.xy[0])}, {int(kpt.xy[1])})", (int(kpt.xy[0]), int(kpt.xy[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
